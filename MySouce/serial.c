...
static rt_err_t rt_serial_init (rt_device_t dev)		//串口设备初始化
{
	struct stm32_serial_device* uart = (struct stm32_serial_device*) dev->user_data;  //取得设备的私有数据

	if (!(dev->flag & RT_DEVICE_FLAG_ACTIVATED)) //判断设备是否是激活状态
	{
		if (dev->flag & RT_DEVICE_FLAG_INT_RX)		 //判断是不是 中断 接收
		{
			rt_memset(uart->int_rx->rx_buffer, 0,	    //这句是干嘛用的？？？？？？？？？？？？？？？？？？？？
				sizeof(uart->int_rx->rx_buffer));
			uart->int_rx->read_index = 0;
			uart->int_rx->save_index = 0;
		}

		if (dev->flag & RT_DEVICE_FLAG_DMA_TX)			//判断是不是 DMA 发送
		{
		...
		dev->flag |= RT_DEVICE_FLAG_ACTIVATED;	 //设备的状态 为激活状态
	}
