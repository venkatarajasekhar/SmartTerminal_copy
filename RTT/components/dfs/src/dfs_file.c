...
void createfile(const char *parth)
{
	int fd=0;
	fd=open(parth,O_CREAT,0);
	if (fd<0)
	{
		rt_kprintf("%s \"%s\" fial","Create",parth);
	}
	close(fd);
}
FINSH_FUNCTION_EXPORT(createfile, Create a file)

int renamefile(const char *oldpath, const char *newpath)
{
	int state;
	state=dfs_file_rename(oldpath,newpath);
	return state;
}
FINSH_FUNCTION_EXPORT(renamefile, rename a file)
